...
export const EditGalleryForm: React.FC<EditGalleryFormProps> = ({
  onSubmit,
  isSubmitting,
  onCancel,
  department
  ...
  const { getGalleryItemById } = useDepartmentGallery();
  
  const [formData, setFormData] = useState<EditGalleryFormData>({
    title: '',
    description: '',
  ...
  useEffect(() => {
    const loadGalleryItem = async () => {
      if (!galleryId) {
        toast.error('Gallery item ID is required');
        router.push(`/notices/${department}`);
        return;
      }

      try {
        const item = await getGalleryItemById(parseInt(galleryId));
        if (item && 'title' in item) {
          setFormData({
            title: item.title,
            description: item.description,
            department: item.department,
            eventDate: item.eventDate.split('T')[0], // YYYY-MM-DD 형식으로 변환
    ...
    loadGalleryItem();
  }, [galleryId, getGalleryItemById, router, department]);
              ...
              <div className={`border-2 border-dashed rounded-lg p-6 text-center transition-all duration-300 ${
                ...
                >
                  ...
                  <span className={`text-sm transition-colors duration-300 ${
                    ...
                    {formData.thumbnail ? formData.thumbnail.name : 'Click to upload new thumbnail image'}
                  </span>
                </label>
              </div>
            </div>
